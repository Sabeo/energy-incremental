    ...
    var Decimal =
        ...
        function() {
            ...
            Decimal.prototype.fromString = function(value) {
                if (IGNORE_COMMAS) { value = value.replace(",", ""); } else if (COMMAS_ARE_DECIMAL_POINTS) { value = value.replace(",", "."); }
                ...
                if (ecount < 1) { this.sign = 0;
                    this.layer = 0;
                    this.mag = 0; return this; }
                var mantissa = parseFloat(parts[0]);
                if (mantissa === 0) { this.sign = 0;
                    this.layer = 0;
                    this.mag = 0; return this; }
            ...
            Decimal.prototype.pow10 = function() {
                ...
                */
                ...
                if (a.layer === 0) {
                    ...
                    if (Number.isFinite(newmag) && Math.abs(newmag) > 0.1) { return FC(1, 0, newmag); } else {
                        if (a.sign === 0) { return Decimal.dOne; } else { a = FC_NN(a.sign, a.layer + 1, Math.log10(a.mag)); }
                    }
                }
            ...
            Decimal.prototype.exp = function() {
                ...
                if (this.layer === 0 && this.mag <= 709.7) { return D(Math.exp(this.sign * this.mag)); } else if (this.layer === 0) { return FC(1, 1, Math.log10(Math.E) * this.mag); } else if (this.layer === 1) { return FC(1, 2, Math.log10(0.4342944819032518) + this.mag); } else { return FC(this.sign, this.layer + 1, this.mag); }
            };
            ...
            Decimal.prototype.sqrt = function() {
                if (this.layer === 0) { return D(Math.sqrt(this.sign * this.mag)); } else if (this.layer === 1) { return FC(1, 2, Math.log10(this.mag) - 0.3010299956639812); } else {
